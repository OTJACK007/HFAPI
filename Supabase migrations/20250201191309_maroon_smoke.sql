...
-- Create trigger function to automatically clean up expired requests
CREATE OR REPLACE FUNCTION check_expired_requests()
RETURNS trigger AS $$
BEGIN
  -- Clean up expired requests when accessing the table
  PERFORM cleanup_expired_audit_requests();
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Create trigger that runs cleanup before insert
CREATE TRIGGER cleanup_expired_requests_insert
  BEFORE INSERT ON audit_key_requests
  EXECUTE FUNCTION check_expired_requests();

-- Create trigger that runs cleanup before update  
CREATE TRIGGER cleanup_expired_requests_update
  BEFORE UPDATE ON audit_key_requests
  EXECUTE FUNCTION check_expired_requests();