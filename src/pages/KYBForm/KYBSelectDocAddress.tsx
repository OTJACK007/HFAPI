...
export default function KYBSelectDocAddress({ register, errors, fieldConfig = defaultKYBFieldConfig, onNext, onBack }: Props) {
  ...
  const allAddressDocOptions: AddressDocOption[] = [
    ...
    { key: "leaseAgreement", value: "leaseAgreement", label: "Contrat de bail" }
  ];

  // Filtrer les options en fonction de la configuration
  const availableAddressDocs = allAddressDocOptions.filter(doc => 
    fieldConfig.businessAddressDocTypes[doc.key as keyof typeof fieldConfig.businessAddressDocTypes]
  );
